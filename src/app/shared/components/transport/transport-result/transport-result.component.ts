...
export class TransportResultComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		if (this.tableDatasource) {
			this.lineChartData = [
				{
					data: _.map(this.tableDatasource, 'busses'),
					label: 'Motor coaches and trolly busses',
				},
				{
					data: _.map(this.tableDatasource, 'cars'),
					label: 'Passanger cars',
				},
				{
					data: _.map(this.tableDatasource, 'trams'),
					label: 'Tram and metro',
				},
				{
					data: _.map(this.tableDatasource, 'trains'),
					label: 'Passanger trains',
				},
				{
					data: _.map(this.tableDatasource, 'rail'),
					label: 'Rail freight',
				},
				{
					data: _.map(this.tableDatasource, 'road'),
					label: 'Road freight',
				},
				{
					data: _.map(this.tableDatasource, 'inland'),
